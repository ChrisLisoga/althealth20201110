...
$quantity= $row3['product_quantity'];

$query2=mysqli_query($dbconn,"SELECT * FROM products WHERE product_id='$product_id'") or die (mysqli_error());
$row2=mysqli_fetch_array($query2);
$product_quantity=$row2['product_quantity'];


 mysqli_query($dbconn,"UPDATE products SET product_quantity = product_quantity - $quantity WHERE product_id ='$product_id' AND product_quantity='$product_quantity'");
...
$cart_table = mysqli_query($dbconn,"SELECT sum(total) FROM order_details WHERE user_id='$user_id' AND order_id=''") or die(mysqli_error());
       $cart_count = mysqli_num_rows($cart_table);
        ...
        while ($cart_row = mysqli_fetch_array($cart_table)) {
           ...
           $tax=$total * 0.12;
           $track_number= $user_id.$user_id+1000;
           $shipaddress=$_POST['shipaddress'];
           $city=$_POST['city'];
           $ship_add=$shipaddress .' '. $city;    
           echo '********* Your tracking number: '.$track_number.' | ';  
           echo 'Total: Php'.$total.' | ';
           echo 'Tax: Php'.$tax.' | '; 
           echo 'Shipping Address: '.$ship_add.' *********';

$query = "INSERT INTO ecommercedb.order (user_id, track_number, firstname, lastname, email, contact, shipping_address, order_date, status, totalprice, tax) 
...
mysqli_query ($dbconn,"UPDATE order_details SET total_quantity =$product_quantity - $quantity WHERE product_id ='$product_id' AND total_quantity='' ");           
...
?>
        ...
        <h5>ElegantMe Technology, Inc.</h5>
